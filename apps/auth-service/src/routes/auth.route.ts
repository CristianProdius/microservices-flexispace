...
router.post("/register", registerLimiter, async (req, res) => {
  try {
    ...
    setRefreshCookie(res, tokens.refreshToken, refreshLifetimeMs());
...
router.post("/logout", shouldBeUser, async (req, res) => {
  try {
    ...
    if (refreshToken) {
      ...
      if (payload?.jti && req.userId) {
        await revokeRefreshChain(payload.jti, req.userId);
      }
    }
...
router.post("/refresh", refreshLimiter, async (req, res) => {
  try {
    ...
    setRefreshCookie(res, newRefreshToken, refreshLifetimeMs());
