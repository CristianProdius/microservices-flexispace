...
export const bookingRoute = async (fastify: FastifyInstance) => {
  ...
  fastify.put(
    ...
    async (request, reply) => {
      ...
      try {
        await producer.send("booking.approved", {
          value: {
            bookingId: id,
            guestEmail: booking.guest.email,
        ...
        request.log.error(
          { err, bookingId: id, topic: "booking.approved" },
          "Failed to publish booking.approved event; status updated but guest will not be notified"
        );
      }
