...
} from "./data/iasi-venues.ts";

// Venue-centric Iasi import: one Venue per location, many Spaces; one HOST per
...
const IMPORT_LOCK_ID = BigInt("42751027");
...
export const validateManifest = (v: ReadonlyArray<CuratedVenueSeed> = IASI_VENUES) => validateCuratedVenueSeeds(v);
...
const upsertHosts = async () => {
  ...
  for (const v of IASI_VENUES) if (!seen.has(v.hostSlug)) seen.set(v.hostSlug, v);
...
export const main = async () => {
  validateManifest();
  const totalSpaces = IASI_VENUES.reduce((n, v) => n + v.spaces.length, 0);
  if (hasArg("--validate")) { console.log(`Validated ${IASI_VENUES.length} Iasi venues / ${totalSpaces} spaces.`); return; }
  ...
  for (const v of IASI_VENUES) for (const s of v.spaces) {
  ...
  for (const venue of IASI_VENUES) {
    ...
    const venuePrefix = `spaces/${owner.id.replace(/[^a-zA-Z0-9_-]/g, "_")}/imports/iasi/${slugify(venue.name)}/venue`;
    ...
    for (const space of venue.spaces) {
      const sp = `spaces/${owner.id.replace(/[^a-zA-Z0-9_-]/g, "_")}/imports/iasi/${slugify(venue.name)}/${slugify(space.name)}`;
  ...
  console.log(`Done. ${venueCount} Iasi venues / ${spaceCount} spaces imported.`);
